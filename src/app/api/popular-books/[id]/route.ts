...
import { popularBookUpdateSchema } from "@/lib/validators/content"

type Params = { params: Promise<{ id: string }> }

export async function GET(_request: Request, { params }: Params) {
  const { id } = await params
  const item = await prisma.popularBook.findUnique({ where: { id } })
...
export async function PUT(request: Request, { params }: Params) {
  ...
  const parsed = popularBookUpdateSchema.safeParse(raw)
  if (!parsed.success) return jsonValidationError(parsed.error)

  try {
    const item = await prisma.popularBook.update({ where: { id }, data: parsed.data })
    return NextResponse.json(item)
  } catch {
...
export async function DELETE(_request: Request, { params }: Params) {
  ...
  const { id } = await params
  try {
    await prisma.popularBook.delete({ where: { id } })
